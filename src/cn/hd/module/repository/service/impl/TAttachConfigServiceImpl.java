...
import cn.hd.module.repository.service.TAttachConfigService;
...
* 动态生成   
*/
@Service("tAttachConfigService")
@Transactional(propagation= Propagation.REQUIRED,readOnly=false,rollbackFor=Exception.class)
public class TAttachConfigServiceImpl extends BasicImplDao<AttachConfigEntity> implements TAttachConfigService {
protected Logger log = Logger.getLogger(this.getClass().getName());
/** 注入持久层 **/
@Autowired
private TAttachConfigDao tAttachConfigDao;
/**
* 保存
* @param tAttachConfig
* @return
...
*/
@Override
public ResponseJSON saveTAttachConfig(AttachConfigEntity tAttachConfig) throws Exception {
ResponseJSON responseJSON = new ResponseJSON();
tAttachConfigDao.save(tAttachConfig);
try{
...
*/
@Override
public ResponseJSON updateTAttachConfig(AttachConfigEntity tAttachConfig) throws Exception {
ResponseJSON responseJSON = new ResponseJSON();
tAttachConfigDao.update(tAttachConfig);
try{
T_xt_rz  rz=new T_xt_rz();
/** 日志记录信息* */
/********** 根据业务不同设置相应的参数值 ***********/
...
*/
@Override
public ResponseJSON delTAttachConfig(AttachConfigEntity tAttachConfig) throws Exception {
ResponseJSON responseJSON = new ResponseJSON();
tAttachConfigDao.delete(tAttachConfig);
try{
T_xt_rz  rz=new T_xt_rz();
/** 日志记录信息* */
/********** 根据业务不同设置相应的参数值 ***********/
...
result.put("total", tAttachConfigDao.count(countHql.toString(),paramMap));
result.put("rows", tAttachConfigDao.find(rowsHql.toString(),paramMap,page,rows));
...
hql.append("from TAttachConfig t1");
List<AttachConfigEntity> list = tAttachConfigDao.find(hql.toString(), paramMap);
Map<String,Object> repMap = new HashMap<String,Object>();
repMap.put("rows",list);
responseJSON.setResult(repMap);
responseJSON.setMsg("查询成功");
try{
log.debug("日志记录...............");
T_xt_rz  rz=new T_xt_rz();

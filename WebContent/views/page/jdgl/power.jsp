...
<head>
    ...
    <script>
            ...
            $.ajax({
                ...
                success: function(msg){
                    ...
                    $.each(msg.rows,function(idx,row){
                        //操作人  操作时间 操作内容
                        var gridBody = "";
                        var userNames = row.userNames?row.userNames:'';
                        var gridBody = "<tr class='addGrid_data_tr' >"+
                            "               <td>"+(idx+1) +"</td>"+
                            "               <td style='text-align:center;'>"+row.mc+"</td>"+
                            "               <td style='text-align:center;'>"+userNames+"</td>"+
                            "               <td style='text-align:center;'><button class='button add_peo' onclick=\"settingUserWin('"+row.id+"','0')\">添加</button></td>";
                        /*if(row.isHold == 1){
         ...
         * open界面
         **/
        function settingUserWin(id,isHold){
            var laySum = top.layer.open({
                ...
                maxmin : true,
                area: ['820px', '450px'],
                content: contextPath+"/tXtDwController/settingUserWin",
                /*弹出框*/
                cancel: function(index, layero){
                    ...
                    var resJOSN = chidlwin.getSelValue();
                    var getSelValueText = chidlwin.getSelValueText();

                    if(resJOSN == 'empty'){
                        top.layer.msg('至少选择一个用户', {
                            icon: 1,
                         ...
                         *  然后ajax 绑定人员ID
                         **/
                        changeStatus(id,resJOSN,getSelValueText);
                       top.layer.close(index)
                    }
        ...
        function changeStatus(id,resJOSN,getSelValueText){
            /**
             * 修改状态后再刷新
             */
            $.ajax({
